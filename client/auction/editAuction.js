...
function EditAuction({ match }) {
  ...
  useEffect(() => {
    ...
    read(
      ...
      if (data.error) {
        ...
        data.bidStart = getDateString(
          new Date(data.bidStart),
        );
        setAuction(data);
      }
    });
  ...
  const clickSubmit = () => {
    if (auction.bidEnd < auction.bidStart) {
      ...
      auction.itemName
        && auctionData.append('itemName', auction.itemName);
      auction.description
        && auctionData.append(
          'description',
          auction.description,
      ...
      auction.image
        && auctionData.append('image', auction.image);
      auction.startingBid
        && auctionData.append(
          'startingBid',
          auction.startingBid,
      ...
      auction.bidStart
        && auctionData.append('bidStart', auction.bidStart);
      auction.bidEnd
        && auctionData.append('bidEnd', auction.bidEnd);
